...
2015-01-30
16:51
接受到的数据要转换为相应的类（集合）存入存入数据库，这里dictionary到class的转换为人为手工对应，在进行更改的时候要修改这里的
相关内容

19：20
添加采样时间操作单元，对于每一个集合的数据，只要考时间戳索引的，都在集合的最开始添加时间戳，即新添加的采样时间操作单元，
无论是在字节数组还是在类中
添加新的处理，对于接收数据处理的方法：在原始接到数据包按发送时操作单元留样（index_obj）解析的基础上添加在请求操作单元无错误的情况下，
直接根据界面类（集合）调用方法转换，在出错或请求大类中不在同一个小类中部分数据的情况下考虑按操作单元解析并手动转换为类

考虑到提取一段时间内的数据且数据量较大的情况，可能出现分包，后续添加除文件传输外的分包方法

对于请求之后响应回来的完整数据整体转换为相应的类（假如请求的是一个完整的类），下端组织的字节数组必须为同服务器端的类成员顺序，或者
下端如果可以的话，使用同于服务器端的类  -- 归结:即便是下端原子组织，还是看上端发送的操作单元顺序

2015-01-31
15:30
现在没有按字节数组截取然后转换为相应的数据库集合类，如果下端按顺序组织或者从相同类转换为字节数组，这里反向处理；

18:41
将四个集合类中的char或者uchar类型修改为string

2015-02-01
10:45
最小操作单元的处理肯定是类直接转换

对于一些最小操作单元的类在commfuncs.define中和model.simldefine中定义产生重复

气体修正参数中的J值不是传输值，在方法中不初始化，计算中想使用相应操作

14:52
由于K值中的J值不在传输的范围内所以在Addconfig方法中就不再给出初始化

23:08
完成Addconfig方法，siml方法未写，由于addsiml貌似不是或者不全部是数据点表中数据，更像是中间分析量

2015-02-02
12:15
完成所有操作单元小类的提取及因修改造成的web端错误，完成数据处理方法中正常情况下的数据库操作，将所有对define中类的引用转移到对
model.simldefine的操作，并示意的完成为实现mongo_id自动增长对接口的实现

2015-2-05：

今日修改：
添加类库SysLog用以记录日志,日志在debug下log文件夹，日志文件格式为日期；每日一志
将终端的设备号和endpoint的数据字典映射到数据库，每次加载系统的时候读到内存，通过心跳更新（已存在的设备ENDPOINT变化了）或者新加(新设备加入)
数据库更新的方法
数据库的操作全部加上try-catch异常捕获
更改到DBService的try-catch
try语句中不能修改外部定义的变量，内部执行必须隔离定义并在范围外不在使用，但是对于静态的变量可进行操作

需要进入日志的项目：
1、凡是调用写的操作；	（只在两个文件出现，一个是wcfserver的commonservices 一个是wrapper中，都有各自的LogException方法写入，
也即只
有两种日志记录方式）

2015-03-07
日志更改：imserverudp中的简历套接字和绑定端口的异常检测释放；
讨论array.copy的异常处理是否捕获的问题
返回的错误如何处理，一方面日志记录
只处理messagebox.show的异常
{
messagebox.show的异常：换做try-finally结构 return
                    try
                    {
                        return 44;
                    }
...
数据入库要不要考虑记录日志
改到第一个子函数在handledata中
数据文件和配置文件不要混放在一起   之类的问题都要注意

修改other    可能在组包和解包的时候有错误
要写报警报文的处理  查验修改为other后是否有错误（解包）   添加具体心跳协议    报告到web端    数据入库  
是否需要减去提取简要等一系列缓存



这里不论前台请求什么内容，可以直接添加到发送队列，每一个界面对应一个标志位，有多中标志码：-1表示存在错误；0表示
循环标志；1表示交互完成，可以存取数据库；这里的

暂时注释掉服务器端接收到心跳包的回发

!!!注：数据备份模块的定时器问题，注释掉定时器的开启,global.asax.cs-line 52
