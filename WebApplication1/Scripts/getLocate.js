    ...
    CityList.prototype._initContainer = function () {
        ...
        baidu.on(provinceDom, "change", function () {
            ...
            if (s_value == 131 || s_value == 289 || s_value == 332 || s_value == 132) {
                ...
                $("#MyContainer").data("MyCity", this.selectedOptions[0].text);
                ////////////////////////////或者用this.selectedOptions.item().text
            } else {
                $("#MyContainer").removeData();
                if (this.selectedOptions[0].text != "请选择")
                {
                    $("#MyContainer").data("MyProvince", this.selectedOptions[0].text);
                }               
...
$(function () {
    ...
    $("#createNewSiteMap").width(window.innerWidth * 0.8 + 'px');  //设置地图宽度
    
    var map2 = new BMap.Map("createNewSiteMap"); //添加地图到页面    
    var point = new BMap.Point(108.99985606913, 34.259565188471);
    map2.centerAndZoom(point, 5);
    map2.enableScrollWheelZoom();
    var myCityList = new BMapLib.CityList({ container: "MyContainer", map: map2 });
    $("select").width(100).height(27); //修改下拉列表的宽度，高度


    $("#MainContent_SearchLocate").click(function () {
        //判断用户未填写信息，则提醒填写
        if ($("#MainContent_MyText").val() == "") { alert("请输入地址信息"); }
        else {         
       
            //向百度API查询——用户所填写地址所在的经纬度
            $.ajax({
                type: "GET",
                url: "http://api.map.baidu.com/geocoder/v2/?" + "ak=" + "OY0PC22e3GNpu0vm2yOKGT6r" + "&output=json" + "&address=" + $("#MainContent_MyText").val() + "&city="+$('#MyContainer').data('MyCity'),
                dataType: "jsonp",
                async: false,
                ...
                success: function (data) {
                    ...
                    function getAttr() {
                        ...
                        $("#MainContent_ctl00___Lng_TextBox1").val(p.lng);
                        $("#MainContent_ctl00___Lat_TextBox1").val(p.lat);
                    }


                    map2.centerAndZoom(point, 19);
                    alert("经度："+lng+"   纬度："+lat);
                    
                    //根据用户选择的列表项，和填入的内容，拼接地址
                    var address = "";
                    if ($("#MyContainer").data("MyProvince") != null )
                    {
                        address = address.concat($("#MyContainer").data("MyProvince"));
                    }
                    if ($("#MyContainer").data("MyCity") != null) {
                        address = address.concat($("#MyContainer").data("MyCity"));
                    }                   
                    address = address.concat($("#MainContent_MyText").val());

                    //给弹出窗口填入地址，经度，纬度
                    $("#MainContent_ctl00___Address_TextBox1").val(address);
                    $("#MainContent_ctl00___Lng_TextBox1").val(marker.getPosition().lng);
                    $("#MainContent_ctl00___Lat_TextBox1").val(marker.getPosition().lat);
